    ...
    async.waterfall([
            ...
            redisClient.zrange(config.coin + ':blocks:candidates', 0, -1, 'WITHSCORES', function(error, results){
                ...
                for (var i = 0; i < results.length; i += 2){
                    ...
                    blocks.push({
                        ...
                        difficulty: parts[2],
                        shares: parts[3],
                        score: parts.length >= 5 ? parts[5] : parts[3]
                    });
                }
        ...
        function(blocks, callback){
            async.filter(blocks, function(block, mapCback){
                ...
                apiInterfaces.rpcDaemon('getblock', {height: blockHeight}, function(error, result){
                    ...
                    block.reward = minerTx.vout[0].amount;
        ...
        function(blocks, callback){
            ...
            blocks.forEach(function(block){
                ...
                log('info', logSystem, 'Unlocked %d block with reward %d and pool fee %d. Miners reward: %d', [block.height, block.reward, feePercent, reward]);
