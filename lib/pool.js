...
var cnBlobType = config.cnBlobType || 10;
var cycleLength = 40;
...
Miner.prototype = {
	...
	getJob: function(){
		if (this.job != null && this.lastBlockHeight === currentBlockTemplate.height && !this.pendingDifficulty && this.cachedJob !== null) {
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage, setMinerid){
	...
	switch(method){
		case 'login':
			...
			var difficulty = parseInt(portData.difficulty);
			...
			if(config.poolServer.fixedDiff.enabled) {
				...
				if (fixedDiffCharPos !== -1 && (login.length - fixedDiffCharPos < 32)){
					...
					if (!difficulty || difficulty != diffValue) {
						...
						difficulty = parseInt(portData.difficulty);
					} else {
						noRetarget = true;
						if (difficulty < config.poolServer.varDiff.minDiff) {
							difficulty = parseInt(config.poolServer.varDiff.minDiff);
						}
					}
				}
			}
		...
		case 'submit':
			...
			if (!miner.job || miner.job.id != params.job_id.toString()){
				sendReply({code: -32503, message: "stale"}, null, 'submit');
				return;
			}

			if ((!params.nonce)||(!params.pow)||(params.pow.length != cycleLength)) {
			...
			if (miner.job.submissions.indexOf(cycle) !== -1){
				var minerText = miner ? (' ' + miner.login + '@' + miner.ip) : '';
			...
			miner.job.submissions.push(cycle);

			var shareAccepted = processShare(miner, params);
 ...
 **/
function recordShareData(miner, height, j_difficulty, j_score, shareDiff, blockCandidate, hashHex, b_difficulty){
	...
	if (slushMiningEnabled) {
		...
		updateScore = ['eval', `
			...
			/* args */ miner.login, j_difficulty, Date.now(), config.poolServer.slushMining.weight];
	}
	else {
		j_score = j_difficulty;
		updateScore = ['hincrbyfloat', config.coin + ':scores:roundCurrent', miner.login, j_score]
	}

	var redisCommands = [
		updateScore,
		['hincrby', config.coin + ':shares_actual:roundCurrent', miner.login, j_difficulty],
		['zadd', config.coin + ':hashrate', dateNowSeconds, [j_difficulty*cycleLength, miner.login, dateNow].join(':')],
		['hincrby', config.coin + ':workers:' + miner.login, 'hashes', j_difficulty*cycleLength],
	...
	if (miner.workerName) {
		redisCommands.push(['zadd', config.coin + ':hashrate', dateNowSeconds, [j_difficulty*cycleLength, miner.login + '~' + miner.workerName, dateNow].join(':')]);
		redisCommands.push(['hincrby', config.coin + ':unique_workers:' + miner.login + '~' + miner.workerName, 'hashes', j_difficulty*cycleLength]);
	...
	if (blockCandidate){
		...
		redisCommands.push(['rename', config.coin + ':scores:roundCurrent', config.coin + ':scores:round' + height]);
		redisCommands.push(['rename', config.coin + ':shares_actual:roundCurrent', config.coin + ':shares_actual:round' + height]);
		redisCommands.push(['hgetall', config.coin + ':scores:round' + height]);
		redisCommands.push(['hgetall', config.coin + ':shares_actual:round' + height]);
	}
	...
	redisClient.multi(redisCommands).exec(function(err, replies){
		...
		if (slushMiningEnabled) {
			j_score = parseFloat(replies[0][0]);
			var age = parseFloat(replies[0][1]);
			log('info', logSystem, 'Submitted score ' + j_score + ' for difficulty ' + j_difficulty + ' and round age ' + age + 's');
		}
		...
		if (blockCandidate){
			...
			redisClient.zadd(config.coin + ':blocks:candidates', height, [
				hashHex,
				Date.now() / 1000 | 0,
				b_difficulty,
				totalShares,
	...
	log('info', logSystem, 'Accepted share at difficulty %d/%d from %s@%s', [j_difficulty, shareDiff, miner.login, miner.ip]);
}

/**
 * Process miner share data
 **/
function processShare(miner, params){
	var nonce = params.nonce;
	var resultHash = params.result;
	var template = Buffer.alloc(currentBlockTemplate.buffer.length);
	currentBlockTemplate.buffer.copy(template);
	template.writeUInt32BE(miner.job.extraNonce, currentBlockTemplate.reserveOffset);
	var b_difficulty = currentBlockTemplate.difficulty;
	var j_difficulty = miner.job.difficulty;
	var j_score = miner.job.score;
	var s_difficulty=getdifficultyfromhash(cuHashing.c29b_cycle_hash(params.pow));
	var height = miner.job.height;

	var shareBuffer = utils.cnUtil.construct_block_blob(template, bignum(nonce,10).toBuffer({endian : 'little',size : 4}),cnBlobType,params.pow);
	
	var header =  Buffer.concat([utils.cnUtil.convert_blob(shareBuffer,cnBlobType),bignum(nonce,10).toBuffer({endian : 'big',size : 4})]);
	prooferror = cuHashing.c29b(header,params.pow);
	...
	if (s_difficulty >= b_difficulty){

		apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
			if (error){
				log('error', logSystem, 'Error submitting block at height %d from %s@%s, - %j', [height, miner.login, miner.ip, error]);
				recordShareData(miner, height, j_difficulty, j_score , s_difficulty, false, null, b_difficulty);
			}
			else{
				...
				log('info', logSystem,
					...
					[blockFastHash.substr(0, 6), height, miner.login, miner.ip, result]
				);
				recordShareData(miner, height, j_difficulty, j_score , s_difficulty, true, blockFastHash, b_difficulty);
				jobRefresh();
			}
		});
	}

	else if (s_difficulty < j_difficulty){
		log('warn', logSystem, 'Rejected low difficulty share of %s from %s@%s', [s_difficulty, miner.login, miner.ip]);
		return false;
	}
	else{
		recordShareData(miner, height, j_difficulty, j_score , s_difficulty, false, null, b_difficulty);
