...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
	...
	var redisCommands = [
		...
		['zadd', config.coin + ':hashrate', dateNowSeconds, [job.difficulty*40, miner.login, dateNow].join(':')],
		['hincrby', config.coin + ':workers:' + miner.login, 'hashes', job.difficulty*40],
	...
	if (miner.workerName) {
		redisCommands.push(['zadd', config.coin + ':hashrate', dateNowSeconds, [job.difficulty*40, miner.login + '~' + miner.workerName, dateNow].join(':')]);
		redisCommands.push(['hincrby', config.coin + ':unique_workers:' + miner.login + '~' + miner.workerName, 'hashes', job.difficulty*40]);
...
function processShare(miner, job, blockTemplate, params){
	...
	var shareType;
	var hashDiff=0;
	var jobdiff = getdifficultyfromhash(cuHashing.c29b_cycle_hash(params.pow));
	...
	if(prooferror){
		...
		hash=cuHashing.c29b_cycle_hash(params.pow);
		shareType = 'valid';
